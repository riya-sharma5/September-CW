...
export const updateCountry = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!_id) return res.status(400).json({ code: 400, message: "_id is required", data: [] });
    if (!countryName || typeof countryName !== "string" || countryName.trim() === "") {
      return res.status(400).json({ code: 400, message: "Invalid Input", data: [] });
    }
    ...
    if (!country) return res.status(404).json({ code: 404, message: "Country not found", data: [] });
...
export const deleteCountry = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!countryName) return res.status(400).json({ code: 400, message: "countryName is required", data: [] });

    const deleted = await countryModel.findOneAndDelete({countryName: countryName});

    if (!deleted) return res.status(404).json({ code: 404, message: "Country not found", data: [] });
