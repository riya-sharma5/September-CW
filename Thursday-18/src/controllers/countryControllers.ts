...
import countryModel  from "../models/countryModels.ts";
...
export const checkFun = async (req:Request,res:Response) => {
  try {
...
export const deleteCountry = async (req: Request, res: Response) => {
  try {
    ...
    if (!countryName) return res.status(400).json({ message: "countryName is required" });
